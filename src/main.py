...
def executar_pipeline():
    ...
    # Leitura de todos os arquivos de Taxa de Distorção
    df_taxa_distorcao_2019_bruto = ler_taxa_distorcao(
        pth.diretorio_TAXA_DISTORCAO_2019)  
    df_taxa_distorcao_2020_bruto = ler_taxa_distorcao(
        pth.diretorio_TAXA_DISTORCAO_2020)  
    df_taxa_distorcao_2021_bruto = ler_taxa_distorcao(
        pth.diretorio_TAXA_DISTORCAO_2021)
    df_taxa_distorcao_2022_bruto = ler_taxa_distorcao(
        pth.diretorio_TAXA_DISTORCAO_2022) 
    df_taxa_distorcao_2023_bruto = ler_taxa_distorcao(
        pth.diretorio_TAXA_DISTORCAO_2023)
    ...
    # Tratamento de todos os arquivos de Taxa de Distorção
    df_taxa_distorcao_2019_tratado = tratar_taxa_distorcao(
        df_taxa_distorcao_2019_bruto) 
    df_taxa_distorcao_2020_tratado = tratar_taxa_distorcao(
        df_taxa_distorcao_2020_bruto)  
    df_taxa_distorcao_2021_tratado = tratar_taxa_distorcao(
        df_taxa_distorcao_2021_bruto)
    df_taxa_distorcao_2022_tratado = tratar_taxa_distorcao(
        df_taxa_distorcao_2022_bruto)  
    df_taxa_distorcao_2023_tratado = tratar_taxa_distorcao(
        df_taxa_distorcao_2023_bruto)

    # --- CONSOLIDAÇÃO DOS DADOS DE DISTORÇÃO ---
    # Junta todos os anos (2019-2023) em um único DataFrame
    df_taxa_distorcao_final = pd.concat(
        [
            df_taxa_distorcao_2019_tratado,
            df_taxa_distorcao_2020_tratado,
            df_taxa_distorcao_2021_tratado,
            df_taxa_distorcao_2022_tratado,
            df_taxa_distorcao_2023_tratado
        ],
        ignore_index=True
    )

    print('\n* Dados de Taxa de Distorção de 2019 a 2023 foram unificados.')
